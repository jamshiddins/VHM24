...
async function testPostgreSQL(dbUrl) {
  ...
  const testFile = process.env.API_KEY_402 || 'test-pg-connection.js';
  const testCode = `
const { Client } = require('pg');
...
async function testRedis(redisUrl) {
  ...
  // Создаем временный файл для тестирования
  const testFile = process.env.API_KEY_403 || 'test-redis-connection.js';
  const testCode = `
const redis = require('redis');
...
`;
  ...
  fs.writeFileSync(process.env.API_KEY_404 || 'RAILWAY_CONNECTIONS_TEST_REPORT.md', reportContent);
