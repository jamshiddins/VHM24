 ...
 * Запускается командой: node set-railway-service-variables.js
 */
...
function setServiceEnvironmentVariables(serviceName, envFilePath) {
  try {
    ...
    log(`Содержимое файла ${envFilePath}:`, 'info');
    log(envContent.substring(0, 100) + '...', 'info');
    
    const envLines = envContent.split('\n');
    log(`Количество строк в файле: ${envLines.length}`, 'info');
      ...
      if (line.trim() && !line.startsWith('#')) {
        ...
        if (match) {
          ...
          if (key && value) {
            ...
            log(`Найдена переменная: ${key}=${value.substring(0, 10)}...`, 'info');
          }
        }
      }
    }
...
function main() {
  ...
  try {
    ...
    const workerEnvSet = setServiceEnvironmentVariables('worker', path.join(__dirname, '.env.worker'));
    ...
    const schedulerEnvSet = setServiceEnvironmentVariables('scheduler', path.join(__dirname, '.env.scheduler'));
