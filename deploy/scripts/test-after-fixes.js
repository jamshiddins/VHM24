...
class TestRunner {
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          this.recordFail(`Ошибка синтаксиса в файле ${file}: ${error.message}`);
        }
      }
    ...
    try {
      ...
      if (fs.existsSync('package.json')) {
        ...
        try {
          ...
          this.recordFail('Проблемы с зависимостями. Рекомендуется выполнить npm install');
        }
        ...
        try {
          ...
          try {
            ...
            if (auditData.metadata && auditData.metadata.vulnerabilities) {
              ...
              if (totalVulns > 0) {
                this.recordFail(`Обнаружены уязвимости: ${totalVulns} (критических: ${vulns.critical}, высоких: ${vulns.high})`);
              } else {
                this.recordPass('Уязвимости в зависимостях не обнаружены');
              }
            }
          } catch (parseError) {
            this.recordFail(`Ошибка при анализе уязвимостей: ${parseError.message}`);
          }
        }
      } else {
    ...
    try {
      ...
      if (fs.existsSync('.env')) {
        ...
        if (hasSensitiveData) {
          ...
          this.recordPass('.env не содержит незащищенных чувствительных данных');
        }
      } else {
      ...
      if (fs.existsSync('.gitignore')) {
        ...
        if (missingPatterns.length > 0) {
          this.recordFail(`.gitignore не содержит необходимые паттерны: ${missingPatterns.join(', ')}`);
        } else {
  ...
  async performanceCheck() {
    ...
    try {
      ...
      if (largeFiles.length > 0) {
        this.recordFail(`Обнаружены большие файлы (>500KB): ${largeFiles.join(', ')}`);
      } else {
      ...
      if (deepNesting.length > 0) {
        this.recordFail(`Обнаружены директории с глубокой вложенностью (>5): ${deepNesting.join(', ')}`);
      } else {
  ...
  async integrationCheck() {
    ...
    try {
      ...
      if (healthChecks.length > 0) {
        this.recordPass(`Обнаружены health check endpoints: ${healthChecks.length}`);
      } else {
  ...
  findJsFiles() {
    ...
    return glob.sync('**/*.js', { 
      ignore: ['**/node_modules/**', '**/dist/**', '**/coverage/**', '**/backup/**'] 
    });
  }
  ...
  findHealthChecks() {
    ...
    return files.filter(file => {
      try {
        ...
        return content.includes('/health') && (content.includes('app.get') || content.includes('fastify.get'));
      } catch (error) {
        return false;
      }
    });
  ...
  generateReport() {
    const report = {
      ...
      summary: {
        ...
        successRate: Math.round((this.stats.passed / (this.stats.total - this.stats.skipped)) * 100) || 0
      }
    };
