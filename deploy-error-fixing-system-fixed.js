...
async function deploySystem() {
  ...
  try {
    ...
    if (!dockerAvailable) {
      log(
        '⚠️ Docker не обнаружен. Некоторые функции могут быть недоступны.',
        'yellow'
      );
    } else {
...
`;
    ...
    try {
      ...
      if (archiveCreated) {
        ...
        log(
          `📁 Путь к архиву: ${path.join(process.cwd(), archiveName)}`,
          'green'
        );
      } else {
...
`;
    ...
    if (allCopied) {
      ...
      log(
        'Некоторые файлы не были скопированы. Проверьте логи выше.',
        'yellow'
      );
    }
