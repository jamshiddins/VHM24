...
/**
 * VHM24 - Деплой системы исправления ошибок (исправленная версия)
 * 
...
// Функция для создания архива без использования внешних команд
function createArchive(sourceDir, archiveName) {
  // Создаем директорию для архива
  ensureDirectoryExists(archiveName);
  ...
  copyDirRecursive(sourceDir, archiveName);
  return true;
}
...
async function deploySystem() {
  ...
  try {
    ...
    if (!dockerAvailable) {
      log('⚠️ Docker не обнаружен. Некоторые функции могут быть недоступны.', 'yellow');
    } else {
      log('✅ Docker обнаружен', 'green');
    }
    ...
    if (missingFiles.length > 0) {
      ...
      const continueAnyway = await confirm('Продолжить несмотря на отсутствие файлов?');
      if (!continueAnyway) {
    ...
    // Копируем файлы
    const filesToCopy = [
      { source: 'scripts/project-analyzer.js', destination: 'deploy/scripts/project-analyzer.js' },
      { source: 'scripts/auto-fixer.js', destination: 'deploy/scripts/auto-fixer.js' },
      { source: 'scripts/test-after-fixes.js', destination: 'deploy/scripts/test-after-fixes.js' },
      { source: 'fix-all-errors.js', destination: 'deploy/fix-all-errors.js' },
      { source: 'setup-error-fixing-system.js', destination: 'deploy/setup-error-fixing-system.js' },
      { source: 'VHM24_FIX_CHECKLIST.md', destination: 'deploy/docs/VHM24_FIX_CHECKLIST.md' },
      { source: 'VHM24_ERROR_FIXING_SYSTEM.md', destination: 'deploy/docs/VHM24_ERROR_FIXING_SYSTEM.md' },
      { source: 'QUICK_START_ERROR_FIXING.md', destination: 'deploy/docs/QUICK_START_ERROR_FIXING.md' },
      { source: 'VHM24_ERROR_FIXING_EXAMPLES.md', destination: 'deploy/docs/VHM24_ERROR_FIXING_EXAMPLES.md' },
      { source: 'packages/shared/logger/index.js', destination: 'deploy/packages/shared/logger/index.js' }
    ];
...
`;
    ...
    try {
      ...
      const archiveCreated = createArchive(deployDir, archiveName);
      
      if (archiveCreated) {
        log(`\n✅ Архив создан: ${archiveName}`, 'green');
        log(`📁 Путь к архиву: ${path.join(process.cwd(), archiveName)}`, 'green');
      } else {
        log(`\n❌ Ошибка при создании архива`, 'red');
      }
    } catch (error) {
      log(`\n❌ Ошибка при создании архива: ${error.message}`, 'red');
    }
    
    // Создаем скрипт для запуска в Docker (если Docker доступен)
    if (dockerAvailable) {
...
`;
      ...
      fs.writeFileSync(path.join(deployDir, 'run-in-docker.sh'), dockerScript);
      fs.chmodSync(path.join(deployDir, 'run-in-docker.sh'), '755');
      log('✅ Создан скрипт запуска в Docker', 'green');
...
`;
    ...
    // Создаем скрипт для запуска без Docker
...
`;
    ...
    fs.writeFileSync(path.join(deployDir, 'run.sh'), runScript);
    fs.chmodSync(path.join(deployDir, 'run.sh'), '755');
    log('✅ Создан скрипт запуска', 'green');
    
    // Завершение
    if (allCopied) {
      log('\n🎉 ДЕПЛОЙ ЗАВЕРШЕН УСПЕШНО!', 'bright');
      log(`📁 Все файлы скопированы в директорию: ${deployDir}`, 'green');
      log('\nДля запуска системы:', 'cyan');
      log(`  cd ${deployDir}`, 'cyan');
      log('  ./run.sh', 'cyan');
      
      if (dockerAvailable) {
        log('\nДля запуска в Docker:', 'cyan');
        log(`  cd ${deployDir}`, 'cyan');
        log('  ./run-in-docker.sh', 'cyan');
      }
    } else {
