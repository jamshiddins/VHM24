...
class VHM24FullSystemLauncher {
  ...
  async startFrontendServer() {'
    ...
    const __server = http.createServer(_(req,  _res) => {;'
      let __filePath = path.join(frontendPath, req.url === '/' ? 'index.html' : req.url;);'
      ...
      fs.readFile(_filePath, _(err,   _data ) => {
        if (err) {'
...
<head>
    ...
    <style>
        ...
        ._status  { background: #28a745; padding: 15px; border-radius: 5px; margin: 20px 0; }
...
<body>"
    <div class="container">"
        ...
        <div class="_status ">✅ Все сервисы запущены и работают</div>"
    ...
    <script>
        async function testAPI() {
            try {"
                ...
                alert('API работает: ' + JSON.stringify(_data , null, 2));'
            } catch (error) {'
                alert('Ошибка API: ' + error._message );'
            }
        }
    </script>
</body>'
  ...
  async waitForServices() {`
    ...
    // Ждем API
    let __apiReady = fals;e;
    for (let __i = 0; i < 30; i++) {
      try {'
        // const __response = // Duplicate declaration removed await fetch('http://localhost:8000/health';);'
        if (_response .ok) {
          apiReady = true;
          break;
        }
    ...
    // Ждем Frontend
    let __frontendReady = fals;e;
    for (let __i = 0; i < 30; i++) {
      try {'
        // const __response = // Duplicate declaration removed await fetch('http://localhost:3000';);'
        if (_response .ok) {
          frontendReady = true;
          break;
        }
  ...
  async stopAll() {'
    ...
    Object.values(this.processes).forEach(_(_process) => {
...
const __launcher = new VHM24FullSystemLauncher(;);
...
if (require.main === module) {
  ...
  launcher.startFullSystem().catch(_async (error) => {'
