...
async function checkDatabaseConnection() {
  ...
  if (!databaseUrl) {
    console.error('❌ DATABASE_URL_PUBLIC и DATABASE_URL не настроены в переменных окружения');
    return false;
  }
  
  // Создаем клиент PostgreSQL с явными параметрами подключения
  const connectionString = databaseUrl;
  console.log(`Строка подключения: ${connectionString}`);
