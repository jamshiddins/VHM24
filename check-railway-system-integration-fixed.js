...
function checkRailwaySettings() {
  ...
  try {
    // Исправлено: используем путь к текущей директории вместо родительской
    ...
    if (fs.existsSync(railwayJsonPath)) {
      ...
      log('Файл railway.json не найден, создаем базовый файл', 'WARNING');
      
      // Создаем базовый railway.json, если он не существует
      const basicRailwayJson = {
        "name": "vhm24",
        "plugins": ["postgresql", "redis", "web"],
        "env": {
          "NODE_ENV": "production"
        }
      };
      
      fs.writeFileSync(railwayJsonPath, JSON.stringify(basicRailwayJson, null, 2));
      log('Создан базовый файл railway.json', 'SUCCESS');
    }
  } catch (error) {
...
async function main() {
  ...
  // Проверяем, запущен ли скрипт локально или в Railway
  if (process.env.NODE_ENV === 'development') {
    log('Запуск в режиме разработки, пропускаем проверку подключения к PostgreSQL', 'WARNING');
    log('В локальной среде адрес postgres.railway.internal недоступен', 'INFO');
    log('Для проверки подключения к PostgreSQL запустите скрипт в Railway', 'INFO');
    isPostgresConnected = true; // Считаем, что подключение успешно для локального запуска
  } else {
    isPostgresConnected = await testPostgresConnection();
  }
