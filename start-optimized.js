...
try {
  ...
  console.error(
    `${ERROR_PREFIX} Ошибка при выполнении emergency-fix.js: ${error.message}`
  );
}
...
try {
  ...
  console.error(
    `${ERROR_PREFIX} Ошибка при освобождении портов: ${error.message}`
  );
}
...
if (!fs.existsSync('.env')) {
  ...
  try {
    ...
    console.error(
      `${ERROR_PREFIX} Ошибка при создании .env файла: ${error.message}`
    );
    process.exit(1);
  }
}
...
try {
  ...
  console.error(
    `${ERROR_PREFIX} Ошибка при проверке переменных окружения: ${error.message}`
  );
}
  ...
  switch (command) {
    ...
    case 'r':
      if (args[1]) {
        ...
        if (serviceIndex !== -1) {
          console.log(
            `${LOG_PREFIX} Перезапуск сервиса ${services[serviceIndex].color}${serviceName}${RESET_COLOR}...`
          );
    ...
    case 's':
      ...
      services.forEach((service, index) => {
        ...
        console.log(
          `${service.color}${service.name}${RESET_COLOR}: ${status} (порт ${service.port})`
        );
      });
      break;
