...
async function fixSyntaxErrors() {
  ...
  const fixes = {
    ...
    routes: async filePath => {
      ...
      content = content.replace(
        /const patchRoutesIdSchema = \{/g,
        'const patchRoutesIdSchema = {'
      );
      // Исправить использование в роуте
      content = content.replace(
        /schema: patchRoutesIdSchema/g,
        'schema: patchRoutesIdSchema'
      );
      if (!content.includes('const logger = ')) {
        content = 'const logger = console;\n' + content;
      }
...
async function fixWebDashboard() {
  ...
  await fs.writeFile(
    path.join(dashboardPath, 'postcss.config.js'),
    postcssConfig,
    'utf8'
  );
...
module.exports = {
  ...
  try {
    ...
    await fs.writeFile(
      path.join(dashboardPath, 'tailwind.config.js'),
      tailwindConfig,
      'utf8'
    );
...
async function main() {
  try {
    ...
    console.log(
      '3. Примените миграции: npx prisma migrate deploy --schema=packages/database/prisma/schema.prisma'
    );
