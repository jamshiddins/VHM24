  ...
  try {
    ...
    if (response.data.status === 'ok') {
      ...
      try {
        ...
        log(
          `Результат проверки дефицита товаров: ${JSON.stringify(inventoryCheckResponse.data)}`,
          colors.green
        );
      } catch (error) {
        log(
          `Ошибка при проверке дефицита товаров: ${error.message}`,
          colors.red
        );
        if (error.response) {
          log(
      ...
      try {
        ...
        log(
          `Результат создания задач ТО: ${JSON.stringify(maintenanceResponse.data)}`,
          colors.green
        );
      } catch (error) {
      ...
      try {
        ...
        log(
          `Результат создания задач инвентаризации: ${JSON.stringify(inventoryResponse.data)}`,
          colors.green
        );
      } catch (error) {
        log(
          `Ошибка при создании задач инвентаризации: ${error.message}`,
          colors.red
        );
        if (error.response) {
          log(
  ...
  for (const pkg of requiredPackages) {
    try {
      ...
      try {
        ...
        log(
          `❌ Не удалось установить пакет ${pkg}: ${installError.message}`,
          colors.red
        );
