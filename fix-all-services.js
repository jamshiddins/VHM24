...
const fixes = [
    ...
    fix: async (content) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('} catch (error) {') && i > 0) {
          ...
          if (!lines[i-1].trim().endsWith(');') && !lines[i-1].trim().endsWith(')')) {
            lines[i-1] = lines[i-1] + ')';
          }
        }
      }
  ...
  },
  {
    ...
    fix: async (content) => {
      ...
      return content.replace(/const patchroutes:idSchema = {/g, 'const patchRoutesIdSchema = {');
    }
  },
  {
    service: 'warehouse',
  ...
  },
  {
    ...
    fix: async (content) => {
      ...
      if (!content.includes("const logger = require")) {
        ...
        for (let i = 0; i < lines.length; i++) {
          if (lines[i].includes('require(') || lines[i].includes('const fastify')) {
            insertIndex = i + 1;
      ...
      return content.replace(/logger\.info/g, 'console.log').replace(/logger\.error/g, 'console.error');
    }
  },
  {
  ...
  },
  {
    ...
    fix: async (content) => {
      ...
      return content.replace(/logger\.error/g, 'console.error').replace(/logger\.info/g, 'console.log');
    }
  }
];
