...
function checkEnvironmentVariables(serviceName, config, envVars) {
  console.log(`${colors.cyan}🔍 Проверка переменных окружения для ${serviceName}:${colors.reset}`);
  ...
  if (config.optional && config.optional.length > 0) {
    ...
    config.optional.forEach(varName => {
      if (envVars[varName]) {
        ...
        console.log(`    ${colors.yellow}!${colors.reset} ${varName} - не установлена`);
        hasWarnings = true;
      }
    });
  }
...
function validateSpecificValues(envVars) {
  ...
  if (envVars.NODE_ENV) {
    ...
    if (validNodeEnvs.includes(envVars.NODE_ENV)) {
      console.log(`    ${colors.green}✓${colors.reset} NODE_ENV: ${envVars.NODE_ENV}`);
    } else {
      console.log(`    ${colors.red}✗${colors.reset} NODE_ENV: недопустимое значение "${envVars.NODE_ENV}"`);
  ...
  if (envVars.PORT) {
    ...
    if (isNaN(port) || port < 1 || port > 65535) {
      console.log(`    ${colors.red}✗${colors.reset} PORT: недопустимое значение "${envVars.PORT}"`);
      hasErrors = true;
    } else {
  ...
  if (envVars.DATABASE_URL) {
    if (envVars.DATABASE_URL.startsWith('postgresql://') || 
      ...
      console.log(`    ${colors.green}✓${colors.reset} DATABASE_URL: корректный PostgreSQL URL`);
    } else {
      console.log(`    ${colors.yellow}!${colors.reset} DATABASE_URL: возможно некорректный формат`);
    }
  }
  ...
  if (envVars.JWT_SECRET) {
    if (envVars.JWT_SECRET.length < 32) {
      console.log(`    ${colors.yellow}!${colors.reset} JWT_SECRET: рекомендуется использовать ключ длиной не менее 32 символов`);
    } else {
      console.log(`    ${colors.green}✓${colors.reset} JWT_SECRET: достаточная длина`);
    }
  }
...
// Основная функция
function main() {
  console.log(`${colors.magenta}🚀 VHM24 - Проверка переменных окружения${colors.reset}\n`);
  ...
  Object.entries(envConfig).forEach(([serviceName, config]) => {
    const { hasErrors, hasWarnings } = checkEnvironmentVariables(serviceName, config, allEnvVars);
  ...
  if (totalErrors === 0 && totalWarnings === 0) {
    console.log(`${colors.green}✅ Все переменные окружения настроены корректно!${colors.reset}`);
    return true;
  } else {
    if (totalErrors > 0) {
      console.log(`${colors.red}❌ Найдено критических ошибок: ${totalErrors}${colors.reset}`);
    }
    if (totalWarnings > 0) {
      console.log(`${colors.yellow}⚠️  Найдено предупреждений: ${totalWarnings}${colors.reset}`);
    }
