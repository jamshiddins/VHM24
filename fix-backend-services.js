...
async function generatePrismaForAllServices() {
  ...
  for (const service of services) {
    ...
    try {
      ...
      const originalSchemaPath = path.join(__dirname, 'packages/database/prisma/schema.prisma');
      ...
      try {
        await exec(`npx prisma generate --schema=${schemaPath}`, { cwd: servicePath });
...
async function installServiceDependencies() {
  ...
  for (const service of ['auth', 'gateway', 'recipes', 'notifications', 'audit', 'monitoring']) {
