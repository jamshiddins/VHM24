/**
 * Скрипт для проверки подключения к базе данных Railway через публичный URL
 */
...
async function checkDatabaseConnection() {
  console.log('=== ПРОВЕРКА ПОДКЛЮЧЕНИЯ К БАЗЕ ДАННЫХ RAILWAY ЧЕРЕЗ ПУБЛИЧНЫЙ URL ===');
  
  // Используем DATABASE_URL_PUBLIC для подключения к базе данных
  const databaseUrl = process.env.DATABASE_URL_PUBLIC;
  
  // Проверяем наличие переменной окружения DATABASE_URL_PUBLIC
  if (!databaseUrl) {
    console.error('❌ DATABASE_URL_PUBLIC не настроен в переменных окружения');
    return false;
  }
