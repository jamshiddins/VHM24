...
const config = {
  services: [
    { name: 'gateway', port: process.env.GATEWAY_PORT || 8000, path: 'services/gateway/src/index.js' },
    { name: 'auth', port: process.env.AUTH_PORT || 3001, path: 'services/auth/src/index.js' },
    { name: 'machines', port: process.env.MACHINES_PORT || 3002, path: 'services/machines/src/index.js' },
    { name: 'inventory', port: process.env.INVENTORY_PORT || 3003, path: 'services/inventory/src/index.js' },
    { name: 'tasks', port: process.env.TASKS_PORT || 3004, path: 'services/tasks/src/index.js' },
    { name: 'bunkers', port: process.env.BUNKERS_PORT || 3005, path: 'services/bunkers/src/index.js' },
    { name: 'backup', port: process.env.BACKUP_PORT || 3007, path: 'services/backup/src/index.js' },
    { name: 'telegram-bot', port: null, path: 'services/telegram-bot/src/index.js' }
  ],
  requiredEnvVars: [
    'JWT_SECRET',
    'DATABASE_URL',
...
function checkDatabaseUrl() {
  ...
  try {
    ...
    console.log(`✅ URL базы данных корректен: ${dbUrl.protocol}//${dbUrl.host}`);
    return true;
  } catch (error) {
...
function checkRedisUrl() {
  ...
  try {
    ...
    console.log(`✅ URL Redis корректен: ${redisUrl.protocol}//${redisUrl.host}`);
    return true;
  } catch (error) {
...
function checkBackupDirectory() {
  ...
  if (!exists) {
    try {
      ...
      console.error('❌ Не удалось создать директорию для бэкапов:', error.message);
    }
  } else {
...
function checkUploadsDirectory() {
  ...
  if (!exists) {
    try {
      ...
      console.error('❌ Не удалось создать директорию для загрузок:', error.message);
    }
  } else {
...
✅ Успешно: ${successCount}/${totalCount} (${successRate}%)
  ...
  if (successRate === 100) {
    ...
    console.log('🔔 Система в целом готова к деплою, но есть некоторые проблемы.');
  } else {
    console.log('⚠️ Система не готова к деплою. Необходимо исправить проблемы.');
  }
}
