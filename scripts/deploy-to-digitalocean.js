...
async function createDoApp() {
  console.log(
    `🔄 Создание приложения ${config.projectName} в DigitalOcean App Platform...`
  );
  ...
  try {
    ...
    if (stdout.includes(config.projectName)) {
      console.log(
        `⚠️ Приложение ${config.projectName} уже существует в DigitalOcean App Platform`
      );
      return true;
    }
...
`;
    ...
    console.log(
      `✅ Приложение ${config.projectName} создано в DigitalOcean App Platform`
    );
...
async function setupEnvironmentVariables() {
  ...
  try {
    ...
    const appIdMatch = stdout.match(
      new RegExp(`([a-z0-9-]+)\\s+${config.projectName}`)
    );
...
async function createSpaces() {
  ...
  try {
    ...
    if (!stdout.includes('vhm24-uploads')) {
      await execAsync(
        `doctl spaces create vhm24-uploads --region ${config.region}`
      );
    ...
    if (!stdout.includes('vhm24-backups')) {
      await execAsync(
        `doctl spaces create vhm24-backups --region ${config.region}`
      );
