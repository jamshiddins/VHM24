...
console.log('🔧 Комплексное исправление всех проблем backend сервисов VHM24...\n');
...
async function createUnifiedPrismaClient() {
  ...
  try {
    ...
    // Копируем схему
    const schemaSource = path.join(__dirname, 'packages/database/prisma/schema.prisma');
    ...
    await exec('npx prisma generate --schema=schema.prisma', { cwd: unifiedClientPath });
...
async function updateServicesToUseUnifiedClient() {
  ...
  for (const service of services) {
    try {
      ...
      packageJson.dependencies['@vhm24/prisma-client'] = 'file:../../packages/prisma-client';
...
`;
  ...
  await fs.writeFile(path.join(__dirname, 'test-prisma-connection.js'), testScript);
