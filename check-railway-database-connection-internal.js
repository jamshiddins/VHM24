/**
 * Скрипт для проверки подключения к базе данных Railway через внутренний URL
 */
...
async function checkDatabaseConnection() {
  console.log('=== ПРОВЕРКА ПОДКЛЮЧЕНИЯ К БАЗЕ ДАННЫХ RAILWAY ЧЕРЕЗ ВНУТРЕННИЙ URL ===');
  
  // Используем DATABASE_URL_INTERNAL для подключения к базе данных
  const databaseUrl = process.env.DATABASE_URL_INTERNAL;
  
  // Проверяем наличие переменной окружения DATABASE_URL_INTERNAL
  if (!databaseUrl) {
    console.error('❌ DATABASE_URL_INTERNAL не настроен в переменных окружения');
    return false;
  }
